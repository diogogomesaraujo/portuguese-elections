...
BEGIN
    WITH parish_base AS (
        ...
        WHERE p.territory_level = 'parish'
          AND lower(unaccent(p.territory_name)) = lower(unaccent(p_parish_name))
         ...
         target AS (
             ...
             FROM parish_base pb
                      JOIN wh.dim_election e
                           ON lower(e.election_type) = lower(p_election_type)
         ...
         final AS (
             SELECT
                 ...
                 COALESCE(pe.color, p_fill) AS winner_color,
                           ...
                           svgshape(
                                   ...
                                   title =>
                                       parish_name
                                           ...
                                           END,
                                   style => svgstyleprop(
                                           stroke => p_stroke::text,
                                           strokewidth => p_strokewidth::text,
                                           fill => COALESCE(winner_color, p_fill)::text,
                                           fillopacity => p_fillopacity::text
                                            )
                           )
