...
DROP FUNCTION IF EXISTS wh.vote_swing_by_territory(text, text, integer, integer, bigint);
               ...
               shares AS (
                   SELECT
                       ...
                       CASE
                           ...
                           END AS margin
                   FROM rolled r
                   -- Filter to only the requested territory when specified
                   WHERE p_territory_key IS NULL
                      OR r.territory_key = p_territory_key
               ),
