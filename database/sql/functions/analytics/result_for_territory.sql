...
WITH wanted_party AS (
    ...
    LIMIT 1
     ...
     vote_territories AS (
         ...
         -- Legislative / Assembly elections → district level
                       ...
                       ON vt.territory_level = 'municipality'
                           AND (
                              (tg.territory_level = 'district'
                                  ...
                                  OR (tg.territory_level = 'municipality'
                                  AND vt.territory_code = tg.territory_code)
                                  OR (tg.territory_level = 'parish'
...
SELECT
    ...
    CASE
        WHEN ranked.total_votes > 0
            THEN ROUND(
                (ranked.votes::numeric / ranked.total_votes::numeric) * 100,
                2
                 )
        END AS vote_pct,
    ...
    CASE
        WHEN ranked.total_seats > 0
            THEN ROUND(
                (ranked.seats::numeric / ranked.total_seats::numeric) * 100,
                2
                 )
        END AS seat_pct,
